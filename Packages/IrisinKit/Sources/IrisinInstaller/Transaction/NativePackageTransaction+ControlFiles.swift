...
extension NativePackageTransaction {
    ...
    func installControlFiles(_ identity: String, archive: NativePackageArchive) throws {
        ...
        for (member, blob) in incoming {
            ...
            try filesystem.backup(path)
            try NativePackageDatabase.write(Data(contentsOf: archive.content(blob)), to: path)
            if ["preinst", "postinst", "prerm", "postrm", "config"].contains(member) {
                guard chmod(path.path, 0o755) == 0 else {
                    throw NativePackageFailure("Cannot set maintainer script permissions")
                }
            }
