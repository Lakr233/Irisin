...
extension NativePackageTransaction {
    ...
    func configure(_ identity: String) throws {
        guard var fields = database.records[identity] else {
            throw NativePackageFailure("Cannot configure absent package: \(identity)")
        }
        guard fields["status"]?.contains("reinstreq") != true else {
            throw NativePackageFailure("Package must be unpacked again before configuration: \(identity)")
        }
        switch NativePackageDatabase.state(of: fields) {
        ...
        default: throw NativePackageFailure("Package is not ready for configuration: \(identity)")
        }
