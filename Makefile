...
check:
	...
	@grep -rnE 'numberOfRowsInSection|numberOfItemsInSection|\.reloadData\(\)|\.performBatchUpdates\(|\.(begin|end)Updates\(\)|(UITableView|UICollectionView)DataSource($$|[^A-Za-z])' --include='*.swift' "$(ROOT_DIR)/Irisin" \
		&& { echo "error: every list is a diffable data source (UITableViewDiffableDataSource / UICollectionViewDiffableDataSource) applying snapshots; no classic data source protocol, no reloadData(), no performBatchUpdates (iOS 16 throws from it on a diffable table)" >&2; exit 65; } || true
