...
check:
	...
	@grep -rnE 'UIActivityViewController\(|popoverPresentationController' --include='*.swift' "$(ROOT_DIR)/Irisin" \
		| grep -vE '/Interface/InterfaceBridge/InterfaceBridge\+ShareSheet\.swift:' \
		&& { echo "error: the share sheet is InterfaceBridge.presentShareSheet(_:anchor:from:), which always gives the iPad's popover somewhere to point; no UIActivityViewController( or popoverPresentationController at a call site" >&2; exit 65; } || true
