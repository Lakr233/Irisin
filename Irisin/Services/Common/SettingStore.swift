...
///
/// The directory is the process's one, so a value of this type is only a
/// handle on it: AptRepository is handed one as its `AptStorage`.
nonisolated struct SettingStore: AptStorage {
    ...
    func read(key: String) -> Data? {
        guard let url = Self.locationFor(key: key) else { return nil }
        return try? Data(contentsOf: url)
    }

    func write(key: String, value: Data?) {
        guard let url = Self.locationFor(key: key) else { return }
        do {
            ...
            Self.lock.withLock { Self.onError }(error.localizedDescription)
        }
    }
}

/// A value persisted through `SettingStore`, cached in memory after the first read.
///
...
@propertyWrapper
final nonisolated class Stored<Value: Codable & Sendable>: @unchecked Sendable {
    ...
    private static func readDisk(key: String) -> Value? {
        guard let data = SettingStore().read(key: key) else { return nil }
        return try? JSONDecoder().decode(Value.self, from: data)
    }

    private func writeDisk(_ newValue: Value) {
        SettingStore().write(key: key, value: try? JSONEncoder().encode(newValue))
    }
}
